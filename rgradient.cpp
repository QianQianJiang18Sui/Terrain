...
RGradient::RGradient() :mLevelCount(350), mColorBufferInvalidated(false), mColorInterpolation(ciRGB)
{
}

RGradient::RGradient(GradientPreset preset): mLevelCount(350), mColorBufferInvalidated(false), mColorInterpolation(ciRGB)
{
    loadPreset(preset);
}
...
QColor RGradient::getColor(double percent)
{
    if (mColorBufferInvalidated) {
        createColorBuffer();
    }

    int index = mLevelCount * percent;
    if (index < 0)
        index = 0;
...
void RGradient::clearColorStops()
{
    mColorStops.clear();
    mColorBufferInvalidated = true;
}

void RGradient::setColorInterpolation(ColorInterpolation interpolation)
{
...
void RGradient::setColorStopAt(double position, const QColor &color)
{
...
void RGradient::loadPreset(GradientPreset preset)
{
    clearColorStops();
    switch (preset)
    {
...
void RGradient::createColorBuffer()
{
